...
class Experiment():
    ...
    def evaluate(cls, score_matrix, train_indices, test_indices):
        ...
        #return {"auroc": auroc,"aupr": aupr,"sample_auroc": sample_auroc,"sample_aupr": sample_aupr}
        return  label,score
    ...
    def run(self, model_cls, comment=None, debug=False):
        ...
        for i, (old_train_indices, test_indices, d_feature, m_feature, shape) in enumerate(self.dataset.split()):
            ...
            label,score = self.evaluate(score_matrix, old_train_indices, test_indices)
            metrics_summary['f1_scores'].append(f1_score(label, score))
            metrics_summary['accuracies'].append(accuracy_score(label, score))
            metrics_summary['recalls'].append(recall_score(label, score))
            metrics_summary['precisions'].append(precision_score(label, score))
            tn, fp, fn, tp = confusion_matrix(label, score).ravel()
            specificity = tn / (tn + fp)
            metrics_summary['specificities'].append(specificity)

            fpr_temp, tpr_temp, _ = roc_curve(label, score)
            ...
            precision_temp, recall_temp, _ = precision_recall_curve(label, score)
