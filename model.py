...
class MicroDiseaseModel(nn.Module):
    def __init__(self, micro_input_dim, micro_hidden_dim, micro_latent_dim,
                 disease_input_dim, disease_hidden_dim, disease_latent_dim, tau=1):
        ...
        self.mic_encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(micro_hidden_dim, micro_latent_dim*2),
            # nn.ReLU(),
            # nn.Linear(256, 64),
            # nn.ReLU(),
        ...
        self.dis_encoder = nn.Sequential(
            nn.Linear(disease_input_dim, disease_hidden_dim),
            nn.ReLU(),
            nn.Linear(disease_hidden_dim, disease_latent_dim*2),
            # nn.ReLU(),
            # nn.Linear(256, 64),
            # nn.ReLU(),
            # nn.Linear(64, latent_dim * 2)  # 输出均值和方差
    ...
    def encode(self, x, encoder):
        ...
        x_out = encoder(x)
        mu, log_var = torch.chunk(x_out, 2, dim=-1)
    ...
    def forward(self, mic_feature_tenor, dis_feature_tensor, mic_i_feature, mic_i_hat_feature, disease_j_feature,
        ...
        mic_latent_tensor = self.encode(mic_feature_tenor.clone(), self.mic_encoder)
        dis_latent_tensor = self.encode(dis_feature_tensor.clone(), self.dis_encoder)

        mic_i_latent_tensor = self.encode(mic_i_feature.clone(), self.mic_encoder)
        mic_i_hat_latent_tensor = self.encode(mic_i_hat_feature.clone(), self.mic_encoder)
        dis_j_latent_tensor = self.encode(disease_j_feature.clone(), self.dis_encoder)
        dis_j_hat_latent_tensor = self.encode(disease_j_hat_feature.clone(), self.dis_encoder)
